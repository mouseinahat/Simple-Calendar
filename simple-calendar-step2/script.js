...
function getCurrentUser() {
  return {
    ...
    color: userColorInput.value || "#4f46e5",
  };
}

function updateUserTheme() {
  const { name, color } = getCurrentUser();
  document.documentElement.style.setProperty("--user-color", color);
  userHint.textContent = `${name} is marking dates with ${color}.`;
  renderCalendar();
  renderSelectedDates();
}
...
function renderSelectedDates() {
  ...
  selectedCount.textContent = `${dates.length} date${dates.length === 1 ? "" : "s"} selected by ${name}`;
...
userNameInput.addEventListener("input", updateUserTheme);
userColorInput.addEventListener("input", updateUserTheme);
